...
<style>.breadcrumb-section {background-image: url({{ asset('storage/uploads/' . $course->course_header_image) }});}</style>
@endif
<?php
            ...
            <div class="row">
                <div class="col-md-9">
                    ...
                    <div class="couse-comment">
                        <div class="blog-comment-area ul-li about-Supervisor-2">
                    ...
                    @if($course->bundles && (count($course->bundles) > 0))
                        <div class="course-details-Client ul-li mt-5">
                                                    ...
                                                    <div class="best-course-pic-text relative-position">
                                                        ...
                                                        <div class="best-course-text">
                                                            ...
                                                            <span class="course-Client"><a
                                                                        href="{{route('courses.Client',['Client'=>$bundle->Client->slug])}}">{{$bundle->Client->name}}</a></span>
                ...
                <div class="col-md-3">
                    <div class="side-bar">
                        ...
                        @if (!$purchased_course)
                                <h3>
                                     @if($course->free == 1)
                                        ...
                                        @lang('labels.frontend.course.price')<span>
                                ...
                                @elseif(auth()->check() && (auth()->user()->hasRole('student')))
                                    ...
                                    @if($course->free == 1)
                                        <form action="{{ route('cart.getnow') }}" method="POST">
                                            ...
                                            <input type="hidden" name="amount" value="{{($course->free == 1 && !\Session::has('user_sposor') && empty(\Session::get('user_sposor'))) ? 0 : $course->price}}"/>
                                    ...
                                    @endif
                                @elseif($logged_in_user->hasRole('Supervisor') || $logged_in_user->isAdmin())
                                @if($logged_in_user->hasRole('Supervisor') && !\DB::table('course_user')->where('user_id',auth()->user()->id)->where('course_id',$course->id)->count())
                                <!-- <h6 class="alert alert-danger"> @lang('labels.frontend.course.buy_note')</h6> -->
                                @else
                                <a href="{{$resume_link}}" class="genius-btn btn-block text-white  {{($course->progress()==100)?'btn-success':'gradient-bg'}} text-center text-uppercase  bold-font">
                        ...
                        @if($global_featured_course != "" && false)
                            <div class="side-bar-widget">
                                ...
                                <div class="featured-course">
                                    <div class="best-course-pic-text relative-position pt-0">
                                        ...
                                        <div class="best-course-text" style="left: 0;right: 0;">
                                            ...
                                            <div class="course-meta">
                                                <span class="course-Client"><a
                                                            href="{{route('courses.Client',['Client'=>$global_featured_course->Client->slug])}}">{{$global_featured_course->Client->name}}</a></span>
