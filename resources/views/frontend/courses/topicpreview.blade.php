            ...
            <div class="row main-content">
                <div class="col-md-9">
                    <div class="course-details-item border-bottom-0 mb-0">
                        @if(isset($topic->topic_image) && !empty($topic->topic_image))
                            <div class="course-single-pic mb30">
                                <img src="{{asset('storage/uploads/'.$topic->topic_image)}}"
                                     alt="">
                            </div>
                        @endif
                        ...
                        @if($topic->mediaVideo && $topic->mediavideo->count() > 0)
                            <div class="course-single-text">
                                @if($topic->mediavideo != "")
                                    <div class="course-details-content mt-3">
                                        <div class="video-container mb-5" data-id="{{$topic->mediavideo->id}}">
                                            @if($topic->mediavideo->type == 'youtube')


                                                <div id="player" class="js-player" data-plyr-provider="youtube"
                                                     data-plyr-embed-id="{{$topic->mediavideo->file_name}}"></div>
                                            @elseif($topic->mediavideo->type == 'vimeo')
                                            ...
                                            @elseif($topic->mediavideo->type == 'upload')
                                                <video poster="" id="player" class="js-player" playsinline controls>
                                                    <source src="{{$topic->mediavideo->url}}" type="video/mp4"/>
                                                </video>
                                            @elseif($topic->mediavideo->type == 'embed')
                                                {!! $topic->mediavideo->url !!}
                                            @endif
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="card p-2 hide">
                            ...
                            <div class="course-single-text mb-5" id="audio_div">
                                <audio id="audioPlayer" {{(Session::has("audiostatus") && Session::get("audiostatus")==1)?'autoplay':''}}>
                                    <source src="{{$topic->mediaAudio->url}}"/>
                                </audio>
                            </div>
                        </div>
                        @endif
                        ...
                        @if(!$topic->mediaAudio)
                        @if(is_file(public_path('/storage/uploads/'.$topic->topic_audio)))
                        <?php 
                            $path = parse_url(public_path('/storage/uploads/'.$topic->topic_audio), PHP_URL_PATH);
                            if(!\App\Models\Media::where('model_id',$topic->id)->count()) {
                                $media=new \App\Models\Media();
                                $media->model_type="App\Models\Topic";
                                $media->model_id=$topic->id;
                                $media->name=basename($path);
                                $media->url=url('/public/storage/uploads/'.$topic->topic_audio);
                        ...
                        <div class="card p-2 hide">
                            ...
                            <div class="course-single-text mb-5">
                                <audio id="audioPlayer" {{(Session::has("audiostatus") && Session::get("audiostatus")==1)?'autoplay':''}}>
                                    <source src="{{url('/public/storage/uploads/'.$topic->topic_audio)}}" type="audio/mp3"/>
                                </audio>
                            </div>
                        </div>
                        @endif
                        ...
                        @if(($topic->downloadableMedia != "") && ($topic->downloadableMedia->count() > 0))
                            <div class="course-single-text mt-4 px-3 py-1 gradient-bg text-white">
                                ...
                                @foreach($topic->downloadableMedia as $media)
                                    <div class="course-details-content text-white">
                                        <p class="form-group">
                                            <a href="{{ route('download',['filename'=>$media->name,'lesson'=>$topic->id]) }}"
                ...
                <div class="col-md-3">
                    <div id="sidebar" class="sidebar">
                        <div class="course-details-Client ul-li">
                            @if(isset($topic->mediaAudio) && $topic->mediaAudio)
...
@push('after-scripts')
    ...
    <script>
        ...
        @if($topic->mediaVideo && $topic->mediaVideo->type != 'embed')
        var current_progress = 0;

        @if($topic->mediaPDF)
        $(function () {
            $("#myPDF").pdf({
                source: "{{asset('storage/uploads/'.$topic->mediaPDF->name)}}",
                loadingHeight: 800,
                loadingWidth: 800,
                loadingHTML: ""
            });
