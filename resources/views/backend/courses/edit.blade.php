        ...
        <div class="card-body">
            ...
            @if (Auth::user()->isAdmin())
                <div class="row">
                    ...
                    <div class="col-10 form-group">
                        {!! Form::label('Supervisors',trans('labels.backend.courses.fields.Supervisors'), ['class' => 'control-label']) !!}
                        {!! Form::select('Supervisors[]', $Supervisors, old('Supervisors') ? old('Supervisors') : $course->Supervisors->pluck('id')->toArray(), ['class' => 'form-control select2', 'multiple' => 'multiple']) !!}
                    </div>
                    <div class="col-2 d-flex form-group flex-column">
                        OR <a target="_blank" class="btn btn-primary mt-auto"
                              href="{{route('admin.Supervisors.create')}}">{{trans('labels.backend.courses.add_Supervisors')}}</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-10 form-group">
                        {!! Form::label('client_id',trans('labels.backend.courses.fields.Client'), ['class' => 'control-label']) !!}
                        {!! Form::select('client_id', $Clients, old('client_id'), ['class' => 'form-control select2 js-example-placeholder-single', 'multiple' => false]) !!}
                    </div>
                    <div class="col-2 d-flex form-group flex-column">
                        OR <a target="_blank" class="btn btn-primary mt-auto"
                              href="{{route('admin.Clients.index').'?create'}}">{{trans('labels.backend.courses.add_Clients')}}</a>
                    </div>
                </div>
            @elseif(Auth::user()->hasRole('Supervisor'))
                <input type="hidden" name="client_id" value="{{(isset($course->client_id))?$course->client_id:''}}" />
            @endif
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-12 col-lg-3 form-group">
                    ...
                    {!! Form::label('course_header_image', trans('labels.backend.courses.fields.course_header_image'), ['class' => 'control-label','accept' => 'image/jpeg,image/gif,image/png']) !!}
                    {!! Form::file('course_header_image', ['class' => 'form-control']) !!}
                    {!! Form::hidden('course_image_max_size', 8) !!}
                    {!! Form::hidden('course_image_max_width', 4000) !!}
                    {!! Form::hidden('course_image_max_height', 4000) !!}
                    @if ($course->course_header_image)
                        <a href="{{ asset('storage/uploads/'.$course->course_header_image) }}" target="_blank"><img
                                    height="50px" src="{{ asset('storage/uploads/'.$course->course_header_image) }}"
                                    class="mt-1"></a>
                    @endif
                </div>
                <div class="col-12 col-lg-3 form-group">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(".js-example-placeholder-single").select2({
                placeholder: "{{trans('labels.backend.courses.select_Client')}}",
            });

            $(".js-example-placeholder-multiple").select2({
                placeholder: "{{trans('labels.backend.courses.select_Supervisors')}}",
            });
        });
