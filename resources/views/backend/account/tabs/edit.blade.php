...
{{ html()->modelForm($logged_in_user, 'PATCH', route('admin.profile.updateVendor'))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
@else
    ...
    <div class="col">
        <div class="form-group">
            ...
            <div>
                ...
                @if(isset($logged_in_user->providers))
...
<div class="row">
    <div class="col">
        <div class="form-group">
            {{ html()->label(__('labels.frontend.user.profile.name'))->for('name') }}

            {{ html()->text('name')
                ->class('form-control')
                ->placeholder(__('labels.frontend.user.profile.name'))
                ->attribute('maxlength', 191)
                ->value($logged_in_user->contact_name)
                ->required() }}
        </div><!--form-group-->
    </div><!--col-->
</div><!--row-->
            ...
            <div class="col">
                <div class="form-group">
                    ...
                    @if(isset($logged_in_user[$item->name]->company_name) && !empty($logged_in_user[$item->name]->company_name))
                        ...
                        <input type="{{$item->type}}" class="form-control mb-0" value="{{$logged_in_user[$item->name]->company_name}}" disabled readonly  
