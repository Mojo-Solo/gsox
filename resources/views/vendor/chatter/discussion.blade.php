		...
		<section id="chatter_header"  style="background:{{ ($discussion->color) ? $discussion->color : 'black	' }}!important;">
			<div class="container breadcrumb-section  relative-position" style="background: inherit" >
				<div class="page-breadcrumb-content text-center px-3">
					...
					<h1>{{ $discussion->title }}</h1><span class="chatter_head_details"> @lang('chatter::messages.discussion.head_details')<a class="chatter_cat" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.Client') }}/{{ $discussion->Client->slug }}" style="background-color:{{ $discussion->Client->color }}">{{ $discussion->Client->name }}</a></span>
				</div>
			</div>
		</section>
                ...
                <div class="col-md-3 left-column">
                    ...
                    <div class="chatter_sidebar">
                        ...
                        <ul class="nav nav-pills nav-stacked">
                            <?php $Clients = DevDojo\Chatter\Models\Models::Client()->all(); ?>
                            @foreach($Clients as $Client)
                                <li><a href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.Client') }}/{{ $Client->slug }}"><div class="chatter-box" style="background-color:{{ $Client->color }}"></div> {{ $Client->name }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                    <!-- END SIDEBAR -->
