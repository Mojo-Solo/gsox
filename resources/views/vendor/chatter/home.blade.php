        ...
        @if(config('chatter.errors'))
            ...
            @if (count($errors) > 0)
                <div class="chatter-alert alert alert-danger">
                    <div class="container">
                        <p><strong><i class="chatter-alert-danger"></i> @lang('chatter::alert.danger.title')
                            </strong> @lang('chatter::alert.danger.reason.errors')</p>
                        <ul>
                ...
                <div class="col-md-3 left-column">
                    ...
                    <div class="chatter_sidebar">
                        <button class="btn btn-primary" id="new_discussion_btn"><i
                                    class="chatter-new"></i> @lang('chatter::messages.discussion.new')</button>
                        <a href="/{{ Config::get('chatter.routes.home') }}"><i
                                    class="chatter-bubble"></i> @lang('chatter::messages.discussion.all')</a>
                        {!! (isset($ClientsMenu))?$ClientsMenu:'' !!}
                    </div>
                    <!-- END SIDEBAR -->
                </div>
                            ...
                            @foreach($discussions as $discussion)
                                <li>
                                    <a class="discussion_list"
                                       href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ (isset($discussion->Client->slug))?$discussion->Client->slug:'' }}/{{ $discussion->slug }}">
                                        ...
                                        @if(Config::get('chatter.user.avatar_image_database_field'))
                                            ...
                                            @else
                                                ...
                                                <span class="chatter_avatar_circle"
                                                      style="background-color:#<?= \DevDojo\Chatter\Helpers\ChatterHelper::stringToColorCode($discussion->user->{Config::get('chatter.user.database_field_with_user_name')}) ?>">
                                        ...
                                        <div class="chatter_middle">
                                            <h3 class="chatter_middle_title">{{ $discussion->title }}
                                                <div class="chatter_cat"
                                                     style="background-color:{{ (isset($discussion->Client->color))?$discussion->Client->color:'' }}">{{ (isset($discussion->Client->name))?$discussion->Client->name:'' }}</div>
                                            </h3>
                                        ...
                                        <div class="chatter_right">
                                            ...
                                            <div class="chatter_count"><i
                                                        class="chatter-bubble"></i> {{ $discussion->postsCount[0]->total }}
                                            </div>
                                        </div>
        ...
        <div id="new_discussion">
            ...
            <form id="chatter_form_editor"
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <!-- Client -->
                        <select id="chatter_client_id" class="form-control" name="chatter_client_id">
                            <option value="">@lang('chatter::messages.editor.select')</option>
                            @if(isset($Clients) && $Clients > 0)
                            @foreach($Clients as $Client)
                                @if(old('chatter_client_id') == $Client->id)
                                    <option value="{{ $Client->id }}" selected>{{ $Client->name }}</option>
                                @elseif(!empty($current_client_id) && $current_client_id == $Client->id)
                                    <option value="{{ $Client->id }}" selected>{{ $Client->name }}</option>
                                @else
                ...
                <div id="new_discussion_footer">
                    ...
                    <button id="submit_discussion" class="btn btn-success pull-right"><i
                                class="chatter-new"></i> @lang('chatter::messages.discussion.create')</button>
