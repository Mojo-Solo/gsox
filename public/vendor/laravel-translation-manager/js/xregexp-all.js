 ...
 * to provide data for Unicode Clients, scripts, blocks, and properties.
 *
 * @requires XRegExp
 */
(function(XRegExp) {
...
/**
 * Adds support for Unicode's general Clients. E.g., `\p{Lu}` or `\p{Uppercase Letter}`. See
 * Client descriptions in UAX #44 <http://unicode.org/reports/tr44/#GC_Values_Table>. Token names
...
(function(XRegExp) {
    ...
    if (!XRegExp.addUnicodeData) {
        throw new ReferenceError('Unicode Base must be loaded before Unicode Clients');
    }
 ...
 * RL1.2 additionally requires support for Unicode scripts and general Clients. These are
 * included in XRegExp's Unicode Clients and Unicode Scripts addons.
 *
 * Token names are case insensitive, and any spaces, hyphens, and underscores are ignored.
 *
 * Uses Unicode 8.0.0.
...
(function(XRegExp) {
    ...
    unicodeData.push({
        name: 'Assigned',
        // Since this is defined as the inverse of Unicode Client Cn (Unassigned), the Unicode
        // Clients addon is required to use this property
        inverseOf: 'Cn'
    });
