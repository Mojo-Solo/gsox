...
if (!function_exists('home_route')) {
    ...
    function home_route()
    {
        ...
        } elseif(auth()->guard('vendor')->check() || (isset(auth()->user()->roles[0]) && auth()->user()->roles[0]->name="manager")) {
            return 'admin.dashboard';
        }
...
if (!function_exists('section_filter')) {
    ...
    function section_filter($section)
    {
        ...
        if ($type == 1) {
            $section_content = \App\Models\Client::has('courses', '>', 7)
                ->where('status', '=', 1)->get()->take(6);
            $section_title = trans('labels.frontend.footer.popular_Clients');
            foreach ($section_content as $item) {
                $single_item = [
                    'label' => $item->name,
                    'link' => route('courses.Client', ['Client' => $item->slug])
                ];
                $content[] = $single_item;
            }
        } else if ($type == 2) {
