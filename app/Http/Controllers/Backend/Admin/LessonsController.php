...
class LessonsController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if(auth()->user()->hasRole('supervisor') && auth()->user()->client_id) {
            $cids = Course::where('client_id',auth()->user()->client_id)->pluck('id');
        } else {
            $cids=Course::ofSupervisor()->pluck('id');
        }
        $lessons = Lesson::whereIn('course_id', $cids);
        if ($request->course_id != "") {
            $lessons = $lessons->where('course_id', (int)$request->course_id)->orderBy('created_at', 'desc')->get();
        }

        if ($request->show_deleted == 1) {
            if (!Gate::allows('lesson_delete') || auth()->user()->hasRole('supervisor')) {
                return abort(401);
            }
    ...
    {
        if (!Gate::allows('topic_view') || auth()->user()->hasRole('supervisor')) {
            //return abort(401);
        }
        $lesson = Lesson::findOrFail($id);
        $course=$lesson->course;
        return view('frontend.courses.lessonpreview', compact('course','lesson'));
    }
