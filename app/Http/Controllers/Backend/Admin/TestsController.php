...
class TestsController extends Controller
{
    ...
    public function index()
    {
        ...
        if(auth()->user()->hasRole('supervisor')) {
            $courses = Course::where('client_id',auth()->user()->client_id)->pluck('title','id')->prepend('Please select', '');
        } else {
            $courses = Course::ofSupervisor()->pluck('title','id')->prepend('Please select', '');
        }
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->course_id > 0) {
            if(auth()->user()->hasRole('supervisor') && Course::where('client_id',auth()->user()->client_id)->where('id',$request->course_id)->count()) {
        ...
        if (request('show_deleted') == 1 && !auth()->user()->hasRole('supervisor')) {
            if (!Gate::allows('test_delete')) {
                return abort(401);
            }
            $tests = Test::onlyTrashed()->get();
        ...
        if (auth()->user()->can('test_edit') || !auth()->user()->hasRole('supervisor')) {
            $has_edit = true;
        }
        if (auth()->user()->can('test_delete') || !auth()->user()->hasRole('supervisor')) {
            $has_delete = true;
        }
    ...
    {
        if (! Gate::allows('test_create') || auth()->user()->hasRole('supervisor')) {
            return abort(401);
        }
        ...
        $lessons = \App\Models\Lesson::where('course_id', $request->course_id)->get()->pluck('title', 'id')->prepend('Please select', '');

        return view('backend.tests.create', compact('courses', 'lessons', 'course_id'));
    }
    ...
    public function store(StoreTestsRequest $request)
    {
        ...
        $test->show_answers = $request->show_answers;
        $test->save();

        $sequence = 1;
        if ($test->course && count($test->course->courseTimeline) > 0) {
            $sequence = $test->course->courseTimeline->max('sequence');
            $sequence = $sequence + 1;
        }

        if ($test->course && $test->published == 1) {
    ...
    {
        if (! Gate::allows('test_edit') || auth()->user()->hasRole('supervisor')) {
            return abort(401);
        }
        $courses = \App\Models\Course::ofSupervisor()->get();
        $courses_ids = $courses->pluck('id');
    ...
    public function update(UpdateTestsRequest $request, $id)
    {
        ...
        $test = Test::findOrFail($id);
        $old_course_id=$test->course_id;
        if($test->course_id && $request->course_id == NULL){
            $timeline = CourseTimeline::where('model_type', '=', Test::class)
                ->where('model_id', '=', $test->id)
        ...
        if ($test->course && $test->published == 1) {
            ...
            $timeline->model_id = $test->id;
            $timeline->model_type = Test::class;
            if($request->course_id!=$old_course_id) {
                $timeline->sequence = $sequence;
            }
            $timeline->save();
        }
    ...
    {
        if (! Gate::allows('test_delete') || auth()->user()->hasRole('supervisor')) {
            return abort(401);
        }
        $test = Test::findOrFail($id);
        $test->delete();
