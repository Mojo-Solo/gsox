...
class TopicsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(auth()->user()->hasRole('supervisor')) {
            $courses = Course::has('client')->where('client_id',auth()->user()->client_id)->pluck('title', 'id')->prepend('Please select', '');
        } else {
            $courses = Course::has('client')->ofSupervisor()->pluck('title', 'id')->prepend('Please select', '');
        }
        if(auth()->user()->hasRole('supervisor')) {
            if(!empty($request->course_id)) {
                $course=Course::where('id',$request->course_id)->where('client_id',auth()->user()->client_id)->firstOrFail();
            }
    ...
    public function getData(Request $request)
    {
        ...
        $has_view = true;
        if (auth()->user()->can('lesson_edit') || !auth()->user()->hasRole('supervisor')) {
            $has_edit = true;
        }
        if (auth()->user()->can('topic_delete') || !auth()->user()->hasRole('supervisor')) {
            $has_delete = true;
        }
    ...
    {
        if (!Gate::allows('lesson_create') || auth()->user()->hasRole('supervisor')) {
            return abort(401);
        }
    ...
    public function store(StoreTopicsRequest $request)
    {
        ...
        $topic = Topic::create($request->except('downloadable_files', 'topic_image','add_audio')
        ...
        if(\Illuminate\Support\Facades\Request::hasFile('topic_image')){
            $file = \Illuminate\Support\Facades\Request::file('topic_image');
            $filename = time() . '-' . $file->getClientOriginalName();
            $path = public_path() . '/storage/uploads/';
            $file->move($path, $filename);
            $url = asset('storage/uploads/' . $filename);
            $topic->topic_image = $url;
            $topic->save();
        }
    ...
    {
        if (!Gate::allows('lesson_edit') || auth()->user()->hasRole('supervisor')) {
            return abort(401);
        }
        $videos = '';
    ...
    public function update(UpdateTopicsRequest $request, $id)
    {
        ...
        $topic->update($request->except('downloadable_files', 'topic_image','add_audio'));
        if (($request->slug == "") || $request->slug == null) {
            $topic->slug = str_slug($request->title);
            $topic->save();
        }
    ...
    public function preview($id)
    {
        ...
        $topic = Topic::findOrFail($id);
        $course=$topic->course;
        return view('frontend.courses.topicpreview', compact('course','topic'));
    }
    ...
    public function uploadfile(Request $request) {
        if($request->hasFile('file')){
            ...
            $file->move($path, $filename);
            $url = asset('storage/uploads/' . $filename);
            return json_encode(array('filename'=>$filename,'url'=>$url));
        }
        return json_encode(array('filename'=>'','url'=>''));
    }
}