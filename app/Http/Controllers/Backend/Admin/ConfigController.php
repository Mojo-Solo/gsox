...
class ConfigController extends Controller
{
    ...
    public function saveGeneralSettings(Request $request)
    {
        ...
        }if ($request->get('option_randomizer') == null) {
            $requests['option_randomizer'] = 0;
        }
    ...
    public function removetimeline($id) {
        $timeline=CourseTimeline::findOrFail($id);
        if($timeline->model_type=="App\Models\Lesson") {
            $lesson=Lesson::find($timeline->model_id);
            if($lesson)
                $lesson->delete();
        }
        if($timeline->model_type=="App\Models\Topic") {
            $topic=Topic::find($timeline->model_id);
            if($topic)
                $topic->delete();
        }
        $timeline->delete();
