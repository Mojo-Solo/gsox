...
class ApiController extends Controller
{
    ...
    {
        $data = ['font_color', 'contact_data', 'counter', 'total_students', 'total_courses', 'total_Supervisors', 'logo_b_image', 'logo_w_image', 'logo_white_image', 'contact_data', 'footer_data', 'app.locale', 'app.display_type', 'app.currency', 'app.name', 'app.url', 'access.captcha.registration', 'paypal.active', 'payment_offline_active'];
        $json_arr = [];
    ...
    public function getLatestNews(Request $request)
    {
        $blog = Blog::orderBy('created_at', 'desc')
            ->select('id', 'client_id', 'user_id', 'title', 'slug', 'content', 'image')
            ->paginate(10);
     ...
     * @return [json] Supervisor object
     */
    public function getSingleSupervisor(Request $request)
    {
        $Supervisor = User::role('Supervisor')->find($request->Supervisor_id);
        if ($Supervisor == null) {
            return response()->json(['status' => 'failure', 'result' => null]);
        }
        $courses = $Supervisor->courses->take(5);
        $bundles = $Supervisor->bundles->take(5);
        return response()->json(['status' => 'success', 'result' => ['Supervisor' => $Supervisor, 'courses' => $courses, 'bundles' => $bundles]]);
    }

    /**
     * Get Supervisor Courses
     *
     * @return [json] Supervisor Courses object
     */
    public function getSupervisorCourses(Request $request)
    {
        $Supervisor = User::role('Supervisor')->find($request->Supervisor_id);
        if ($Supervisor == null) {
            return response()->json(['status' => 'failure', 'result' => null]);
        }
        $courses = $Supervisor->courses()->paginate(10);
        return response()->json(['status' => 'success', 'result' => ['Supervisor' => $Supervisor, 'courses' => $courses]]);
    }

    /**
     * Get Supervisor Bundles
     *
     * @return [json] Supervisor Bundles object
     */
    public function getSupervisorBundles(Request $request)
    {
        $Supervisor = User::role('Supervisor')->find($request->Supervisor_id);
        if ($Supervisor == null) {
            return response()->json(['status' => 'failure', 'result' => null]);
        }
        $bundles = $Supervisor->bundles()->paginate(10);
        return response()->json(['status' => 'success', 'result' => ['Supervisor' => $Supervisor, 'bundles' => $bundles]]);
    }
    ...
    public function getSingleCourse(Request $request)
    {
        ...
        $course = Course::withoutGlobalScope('filter')->with('Supervisors', 'Client')->where('id', '=', $request->course_id)->with('publishedLessons')->first();
        if ($course == null) {
            return response()->json(['status' => 'failure', 'result' => NULL]);
        }
    ...
    public function getBlog(Request $request)
    {
        ...
        if ($request->blog_id != null) {
            ...
            $blog = Blog::with('comments', 'Client', 'author')->find($blog_id);
        ...
        $blog = Blog::has('Client')->with('comments')->OrderBy('created_at', 'desc')->paginate(10);
     ...
     */
    public function getBlogByClient(Request $request)
    {
        $Client = Client::find((int)$request->client_id);
        if ($Client != null) {
            $blog = $Client->blogs()->paginate(10);
            return response()->json(['status' => 'success', 'result' => $blog]);
        }
        return response()->json(['status' => 'failure']);
    ...
    public function getForum(Request $request)
    {
        ...
        $discussions = Models::discussion()->with('user')->with('post')->with('postsCount')->with('Client')->orderBy(config('chatter.order_by.discussions.order'), config('chatter.order_by.discussions.by'));
        if (isset($slug)) {
            $Client = Models::Client()->where('slug', '=', $slug)->first();

            if (isset($Client->id)) {
                $current_client_id = $Client->id;
                $discussions = $discussions->where('chatter_client_id', '=', $Client->id);
            } else {
    ...
    public function createDiscussion(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'chatter_client_id.required' => trans('chatter::alert.danger.reason.Client_required'),
        ]);
        ...
        $new_discussion = [
            'title' => $request->title,
            'chatter_client_id' => $request->chatter_client_id,
            'user_id' => $user_id,
            'slug' => $slug,
            'color' => '#0c0919',
        ];

        $Client = Models::Client()->find($request->chatter_client_id);
        if (!isset($Client->slug)) {
            $Client = Models::Client()->first();
        }
    ...
    public function storeResponse(Request $request)
    {
        ...
        $Client = Models::Client()->find($discussion->chatter_client_id);
        if (!isset($Client->slug)) {
            $Client = Models::Client()->first();
        }
    ...
    public function getMessages(Request $request)
    {
        ...
        $Supervisors = User::role('Supervisor')->get()->pluck('name', 'id');
