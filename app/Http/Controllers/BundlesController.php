...
class BundlesController extends Controller
{
    ...
    public function all()
    {
        ...
        return view( $this->path.'.bundles.index', compact('bundles', 'purchased_bundles', 'recent_news','featured_courses','Clients'));
    }
    ...
    {
        $Client = Client::where('slug', '=', $request->Client)->first();
        if ($Client != "") {
            $recent_news = Blog::orderBy('created_at', 'desc')->take(2)->get();
            $featured_courses = Course::where('published', '=', 1)
                ->where('featured', '=', 1)->take(8)->get();

            $courses = $Client->courses()->where('published', '=', 1)->paginate(9);
            return view( $this->path.'.courses.index', compact('courses', 'Client', 'recent_news','featured_courses'));
        }
        return abort(404);
    }
