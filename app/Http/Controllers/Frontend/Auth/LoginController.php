...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if($validator->passes()){
            ...
            $authSuccess = \Illuminate\Support\Facades\Auth::guard('vendor')->attempt(['email' => $request->email, 'password' => $request->password], $request->has('remember'));
            if($authSuccess) {
                ...
                if($vendor && $vendor->created_by) {
                        $manger=User::where('id',$vendor->created_by)->first();
                        if($manger && $manger->roles[0]->name=='manager') {
                            \Session::put('user_sposor','manager');
                            \Session::save();
                        }
                    }
                ...
                $authSuccess = \Illuminate\Support\Facades\Auth::guard('web')->attempt($credentials, $request->has('remember'));
                if($authSuccess) {
                    ...
                    if(auth()->user()->active > 0){
                        ...
                        if(auth()->user()->vendor_id) {
                            $vendor=\DB::table('vendors')->where('id',auth()->user()->vendor_id)->first();
                            if($vendor && $vendor->created_by) {
                                $manger=User::where('id',$vendor->created_by)->first();
                                if($manger && $manger->roles[0]->name=='manager') {
                                    \Session::put('user_sposor','manager');
            ...
            return response(['success' => false,'message' => 'Login failed. Account not found'], Response::HTTP_FORBIDDEN);
    ...
    public function logout(Request $request)
    {
        ...
        if(\Session::has('user_sposor')) {
            \Session::forget('user_sposor');
            \Session::save();
        }
