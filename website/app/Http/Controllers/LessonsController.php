...
class LessonsController extends Controller
{
    ...
    public function show($course_id, $lesson_slug, $type)
    {
        $test_result = "";
        $completed_lessons = "";
        $item_type ="";
        if($type =='topic'){
            $lesson = Topic::where('slug', $lesson_slug)->where('course_id', $course_id)->where('published', '=', 1)->first();  
        }else{
        ...
        if ((int)config('lesson_timer') == 0 && $type !='topic') {
        ...
        return view($this->path . '.courses.lesson', compact('lesson', 'previous_lesson', 'next_lesson', 'test_result',
            'purchased_course', 'test_exists', 'lessons', 'completed_lessons', 'item_type', 'inline_test_exists', 'inline_test', 'test_status', 'disable_next_btn'));
    }
    ...
    public function retest(Request $request)
    {
        ...
        $get_test = Test::findOrFail($test_id);
        if($get_test && $get_test->fail_redirect !=''){
            return redirect($get_test->fail_redirect);
        }
